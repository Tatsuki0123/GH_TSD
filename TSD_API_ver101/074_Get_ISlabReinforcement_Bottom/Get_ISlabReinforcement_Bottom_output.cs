...
namespace TSD_API_ver101_ISlabReinforcement_Bottom
{
    ...
    {
        // Task -> Object
        public static TSD.API.Remoting.Reinforcement.ISlabReinforcement GetISlabReinforcement_Bottom(TSD.API.Remoting.Structure.ISlabItem islabItem)
        {
            try
            {
                // Call the asynchronous method and wait for its completion non-blockingly
                var resultTask = Task.Run(async () => await Get_ISlabReinforcement_Bottom_process.GetISlabReinforcement_BottomAsync(islabItem));

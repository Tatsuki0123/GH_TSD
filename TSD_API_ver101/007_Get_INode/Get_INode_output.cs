...
namespace TSD_API_ver101_INode
{
    ...
    {
        // Task -> Object
        public static List<TSD.API.Remoting.Solver.INode> GetINode(TSD.API.Remoting.Solver.IModel solIModel)
        {
            try
            {
                // Call the asynchronous method and wait for its completion non-blockingly
                var resultTask = Task.Run(async () => await Get_IElement1D_process.GetINodeAsync(solIModel));

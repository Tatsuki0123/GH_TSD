...
{
    public class Get_stIModel_output
    {
        // Task -> Object
        public static IDocument GetIDoc(IApplication IApp)
        {
            try
            {
                // Call the asynchronous method and wait for its completion non-blockingly
                var resultTask = Task.Run(async () => await Get_stIModel_process.GetDocumentsAsync(IApp));

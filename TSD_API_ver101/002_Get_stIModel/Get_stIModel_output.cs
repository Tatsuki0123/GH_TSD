...
{
    public class Get_solIModel_output
    {
        // Task -> Object
        public static TSD.API.Remoting.Structure.IModel GetstIModel(IDocument IDoc)
        {
            try
            {
                // Call the asynchronous method and wait for its completion non-blockingly
                var resultTask = Task.Run(async () => await Get_solIModel_process.GetstIModelAsync(IDoc));

...
namespace TSD_API_ver101_Create_IDocument
{
    public class Save_IDocument_Process
    {
        ...
        public static async Task<IDocument> CreateIDocumentAsync(IApplication Iapp, bool allowUserInteraction)
        {
            try
            {
                using (CancellationTokenSource cts = new CancellationTokenSource(TimeSpan.FromMinutes(1)))
                {
                    var Idocument = await Iapp.CreateDocumentAsync(allowUserInteraction);
